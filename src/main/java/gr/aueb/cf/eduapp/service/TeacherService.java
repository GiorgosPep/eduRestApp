...
public class TeacherService implements ITeacherService{
    ...
    public TeacherReadOnlyDTO saveTeacher(TeacherInsertDTO dto)
        ...
        if(dto.personalInfoInsertDTO().identityNumber() != null
            ...
            throw new EntityAlreadyExistsException("IdentityNumber", "Teacher with Identity Number = " + dto.personalInfoInsertDTO().identityNumber() +
                    " already exists");
        }
        ...
        Region region = regionRepository.findById(dto.regionId()).orElseThrow(() -> new EntityAlreadyExistsException("Region","Region with id ="
    ...
    public void saveAmkaFile(UUID uuid, MultipartFile amkaFile)
        ...
        attachment.setFilename(newFilePath.toString());
        ...
        try(InputStream is = amkaFile.getInputStream()){
            ...
            throw new FileUploadException("FileUpload","Fail to detect file type");
        }
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCommit() {
                try {
                    ...
                    log.error("Critical: DB transaction commited but file upload failed for path={} ",
                             newFilePath, e);
    ...
    public TeacherReadOnlyDTO updateTeacher(TeacherUpdateDTO dto) throws EntityNotFoundException,
            EntityAlreadyExistsException, EntityInvalidArgumentException {

        Teacher teacher = teacherRepository.findByUuid(dto.uuid()).
                orElseThrow(() -> new EntityNotFoundException("Teacher", "Teacher with uuid = " +
                        dto.uuid() + " does not exist"));
...
//        }
        ...
        teacherRepository.save(teacher);        // προαιρετικό γιατί έχει γίνει dirty check (το Spring τα αποθηκεύει)
    ...
    public TeacherReadOnlyDTO deleteTeacherByUUID(UUID uuid) throws EntityNotFoundException {
        ...
        // No save is needed if Teacher is managed
        // teacherRepostitory.save(teacher);
    ...
    public Page<TeacherReadOnlyDTO> getPaginatedTeachers(Pageable pageable) {
        ...
        log.debug("Get paginated returned successfully, page={}, size={}",
                teacherPage.getNumber(), teacherPage.getSize());
    ...
    public Page<TeacherReadOnlyDTO> getTeachersPaginatedFiltered(Pageable pageable, TeacherFilters filters)
        ...
        if(filters.getUuid() != null) {
            Teacher teacher = teacherRepository.findByUuidDeletedFalse(filters.getUuid())
                    .orElseThrow(() -> new EntityNotFoundException("Teacher", "Teacher with uuid ="
                    + filters.getUuid()));
        ...
        if(filters.getAmka() != null) {
            Teacher teacher = teacherRepository.findByPersonalInfo_Amka(filters.getAmka())
                    .orElseThrow(() -> new EntityNotFoundException("Teacher", "Teacher with amka ="
                            + filters.getAmka()));
            return singleResultPage(teacher, pageable);
        }
        if(filters.getVat() != null) {
            Teacher teacher = teacherRepository.findByvatDeletedFalse(filters.getVat())
                    .orElseThrow(() -> new EntityNotFoundException("Teacher", "Teacher with vat ="
                            + filters.getAmka()));
            return singleResultPage(teacher, pageable);
        }
